...
def getDronecoordinates():
    angle = drone.navdata.demo.psi
    height = drone.navdata.demo.altitude
    x = 0 #!!!!!
    y = 0 #!!!!!
...
def controller(coordinates, points):
    ...
    for x in range(0, len(coordinates)):
       error.append((coordinates[x]-points[x]))
...
def routeplanning(picture):
    ...
    if set(coords).intersection(points[0]):#Compares the coordinates
...
z_pid = PIDcontroller(2, 2, 3, sample_control, 0,0)
